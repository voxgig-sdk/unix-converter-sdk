...
class ConversionEntity
  ...
  def make
    opts = @_entopts.dup
    ConversionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UnixConverterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Conversion, Hash] the current Conversion data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UnixConverterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Conversion fields)
  def match_get
  ...
  #
  # @param reqmatch [ConversionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Conversion.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Conversion, Hash] the loaded Conversion; raises UnixConverterError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UnixConverterHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
