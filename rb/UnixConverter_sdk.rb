...
require_relative 'UnixConverter_types'
...
class UnixConverterSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UnixConverterHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UnixConverterHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UnixConverterFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UnixConverterUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UnixConverterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UnixConverterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UnixConverterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = UnixConverterHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UnixConverterError.new(
        "graphql_error", "UnixConverterSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = UnixConverterSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
