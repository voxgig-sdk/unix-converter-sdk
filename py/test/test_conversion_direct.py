...
class TestConversionDirect:

    def test_should_direct_load_conversion(self):
        setup = _conversion_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-conversion", "live" if setup["live"] else "unit")
        if _skip:
...
def _conversion_direct_setup(mockres):
