...
class UnixConverterTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UnixConverterTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UnixConverterTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UnixConverterTestRunner.getenv("UNIX_CONVERTER_TEST_LIVE")
        override = UnixConverterTestRunner.getenv("UNIX_CONVERTER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UnixConverterTestRunner.getenv(key)
        ...
        explain = UnixConverterTestRunner.getenv("UNIX_CONVERTER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if UnixConverterTestRunner._test_control is not None:
            return UnixConverterTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UnixConverterTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UnixConverterTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UnixConverterTestRunner.load_test_control()
...
def load_env_local():
    UnixConverterTestRunner.load_env_local()


def env_override(m):
    return UnixConverterTestRunner.env_override(m)


def entity_data(v):
    return UnixConverterTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UnixConverterTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UnixConverterTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UnixConverterTestRunner.load_test_control()


def live_client_options():
    return UnixConverterTestRunner.live_client_options()


def live_delay_ms():
    return UnixConverterTestRunner.live_delay_ms()
