...
import (
	...
	omni "github.com/voxgig-sdk/unix-converter-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UnixConverterError); is {
				out := map[string]any{
					"name":    "UnixConverterError",
...
func omniCtx(arg any, client *sdk.UnixConverterSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UnixConverterSDK); is {
			client = live
