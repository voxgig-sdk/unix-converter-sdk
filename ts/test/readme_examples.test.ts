...
//     (\`new UnixConverterSDK(...)\` / \`UnixConverterSDK.test(...)\` become
//     \`UnixConverterSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'UnixConverterSDK'
...
const TEST_SEED = {"entity":{"conversion":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UnixConverterSDK(...)\` and any
// \`UnixConverterSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UnixConverterSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UnixConverterSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
