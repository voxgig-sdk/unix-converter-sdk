...
describe('ConversionDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new UnixConverterSDK({
  ...
  test('direct-load-conversion', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-conversion', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['UNIX_CONVERTER_TEST_CONVERSION_ENTID']
