...
boru-driven command-line client **and** interactive REPL for the UnixConverter
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/unix-converter-cli)
make build

# 2. See usage (words, entities, env vars)
./unix-converter-cli --help

# 3. Provide credentials once, via the environment
export UNIX_CONVERTER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./unix-converter-cli load 1 conversion            # {id:1} shorthand
./unix-converter-cli load '{id:1}' conversion       # explicit match map

# 5. Override the API base URL for a single call
UNIX_CONVERTER_BASE=https://api.example.com ./unix-converter-cli load 1 conversion

# 6. No arguments -> interactive REPL
./unix-converter-cli
unix-converter> load 1 conversion
unix-converter> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/unix-converter-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/unix-converter-cli load 1 conversion
   ```
...
```sh
./unix-converter-cli load 1 conversion          # scalar shorthand for {id:1}
./unix-converter-cli load '{id:1}' conversion     # explicit match map
```
...
./unix-converter-cli load 1 conversion
```
...
Run with no arguments to open a REPL (prompt `unix-converter>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `conversion`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/unix-converter-cli`. |
...
(`./unix-converter-cli <expr>`) and interactively (the REPL), and expressions compose the
