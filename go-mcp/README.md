...
[MCP](https://modelcontextprotocol.io) server exposing the UnixConverter SDK as
two agent tools — `unix-converter_list` and `unix-converter_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/unix-converter-mcp)
make build

# 2. Provide credentials via the environment
export UNIX_CONVERTER_APIKEY=sk_live_xxx
...
```jsonc
// unix-converter_list: first page of records
{ "entity": "conversion" }
{ "entity": "conversion", "query": { } }

// unix-converter_load: one record by id
{ "entity": "conversion", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/unix-converter-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user unix-converter \
     -- "$PWD"/dist/*/unix-converter-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `unix-converter_list` and `unix-converter_load` tools now appear
   in new sessions. Ask the agent to *"list conversion using unix-converter"*
   and it calls `unix-converter_list` with `{"entity":"conversion"}`.
...
```sh
export UNIX_CONVERTER_APIKEY=sk_live_xxx            # API key
export UNIX_CONVERTER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `unix-converter_list` tool
...
### Call the `unix-converter_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `unix-converter_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `unix-converter_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `UNIX_CONVERTER_APIKEY` | API key sent with every request. |
| `UNIX_CONVERTER_BASE` | Optional override of the API base URL. |
