...
module UnixConverterUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UnixConverterResponse.new({})
    ...
    if fetch_err
      ...
      response = UnixConverterResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UnixConverterResponse.new(fetched)
    else
