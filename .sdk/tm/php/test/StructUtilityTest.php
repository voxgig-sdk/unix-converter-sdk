...
// UnixConverter SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = UnixConverterOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(UnixConverterOmni::structfix($now, Struct::undef()))
                        !== json_encode(UnixConverterOmni::structfix($val, Struct::undef()))) {
