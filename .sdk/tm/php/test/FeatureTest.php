...
// UnixConverter SDK feature test
//
...
class FtCtrl extends UnixConverterControl
{
...
class FtClient
{
    ...
    private UnixConverterUtility $utility;

    public function __construct(UnixConverterUtility $utility, array $options)
    {
    ...
    public function get_utility(): UnixConverterUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UnixConverterContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UnixConverterContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UnixConverterContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UnixConverterConfig::make_feature($name);
    ...
    private static function buildUrl(UnixConverterSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UnixConverterError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UnixConverterResponse($fetched);
        }
    ...
    private function fail(UnixConverterContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UnixConverterContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
