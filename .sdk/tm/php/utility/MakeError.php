...
class UnixConverterMakeError
{
    public static function call(?UnixConverterContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new UnixConverterContext([], null);
        }
        $op = $ctx->op ?? new UnixConverterOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new UnixConverterResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof UnixConverterError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new UnixConverterError('', $msg, $ctx);
        ...
        if ($err instanceof UnixConverterError) {
