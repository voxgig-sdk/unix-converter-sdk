...
// UnixConverter SDK retry feature
...
class UnixConverterRetryFeature extends UnixConverterBaseFeature
{
    ...
    public function init(UnixConverterContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UnixConverterContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UnixConverterContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UnixConverterContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
