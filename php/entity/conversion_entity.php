...
// UnixConverter SDK Conversion entity
...
class ConversionEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ConversionEntity($this->_client, $opts);
    }

    /**
     * @param Conversion|array $args Conversion data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = UnixConverterHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Conversion|array The current Conversion data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Conversion fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = UnixConverterHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Conversion fields).
     */
    ...
    /**
     * Load a single Conversion.
     *
     * @param ConversionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ConversionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Conversion|array The loaded Conversion as an assoc-array at the
     *   SDK boundary; throws UnixConverterError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = UnixConverterHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
